...
class DQN():
  # DQN Agent
    ...
    def train_Q_network(self):
        ...
        # Step 2: calculate y
        y_batch = []
        Q_value_batch = self.Q_value.eval(feed_dict={self.state_input: next_state_batch, self.pkeep: 0.75})
        ...
        self.optimizer.run(feed_dict={self.y_input: y_batch, self.action_input: action_batch, self.state_input: state_batch, self.lr: self.learning_rate, self.pkeep: 0.75})

    def egreedy_action(self, observation):
        Q_value = self.Q_value.eval(feed_dict={self.state_input: [observation], self.pkeep: 1.0})[0]
        e = random.random()
        if e <= self.epsilon:
    ...
    def action(self, observation):
        return np.argmax(self.Q_value.eval(feed_dict={self.state_input: [observation], self.pkeep: 1.0})[0])

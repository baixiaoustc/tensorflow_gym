...
class DQN():
  # DQN Agent
    ...
    def train_Q_network(self):
        ...
        Q_value_batch = self.Q_value.eval(feed_dict={self.state_input: next_input_batch})
        ...
        self.optimizer.run(feed_dict={self.y_input: y_batch, self.action_input: action_batch, self.state_input: input_batch})

    def egreedy_action(self, observation_list):
        input = np.array(observation_list).reshape(84, 84, 4)
        Q_value = self.Q_value.eval(feed_dict={self.state_input: [input]})[0]
        e = random.random()
        if e <= self.epsilon:
    ...
    def tack_action(self, observation_list):
        input = np.array(observation_list).reshape(84, 84, 4)
        return np.argmax(self.Q_value.eval(feed_dict={self.state_input: [input]})[0])
